...
handle :: (Ord e, Pretty.Pretty e) => Parser e l a -> (Set e -> Parser e l a) -> Parser e l a
handle ma handler = Parser \s ->
  case ma.unParser s of
    (Nothing, errs, 0) -> (handler errs).unParser s
    other              -> other

die :: (Ord e, Pretty.Pretty e) => Set e -> Parser e l a
die msgs = Parser (const (Nothing, msgs, 0))

instance (Ord e, Pretty.Pretty e) => Alternative (Parser e l) where
  empty = die mempty
...
match :: (Ord e, Pretty.Pretty e) => e -> (l -> Maybe a) -> Parser e l a
match err proj = Parser \case
  [] -> error "[]"
  (proj -> Just a) : _ -> (Just a,  mempty,            1)
  _                    -> (Nothing, Set.singleton err, 0)

sepBy  :: (Ord e, Pretty.Pretty e) => Parser e l a -> Parser e l sep -> Parser e l [a]
sepBy1 :: (Ord e, Pretty.Pretty e) => Parser e l a -> Parser e l sep -> Parser e l (NE.NonEmpty a)
